...
<tr>
<td colspan="5">
<form action="orders.do?method=list" method="post">
...
<c:if test="${o.status eq 4 }">用户已确认</c:if>
</td>
<td>${o.user.nickname }&nbsp;<a href="orders.do?method=userList&uid=${o.user.id }">查询用户订单</a></td>
...
<c:if test="${o.status eq 2 && o.user.id ne loginUser.id }">
<a href="orders.do?method=sendProduct&oid=${o.id }">发货</a>&nbsp;</c:if>
<c:if test="${o.status eq 3 && o.user.id eq loginUser.id }">
...
<jsp:param value="status,method" name="params"/>
</jsp:include>
</td></tr>
</table>
</body>
